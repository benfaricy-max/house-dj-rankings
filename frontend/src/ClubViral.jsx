...
const normalize = (v, min, max) => (max === min ? 0 : ((v - min) / (max - min)) * 100);

// CLUB = scene / DJ / live-booking demand. VIRAL = consumer hype / reach.
const CLUB = [
  { key: "ra_score",        pct100: true },   // RA live-booking strength (already 0-100)
...
function blend(dj, defs, ranges) {
  ...
  for (const d of defs) {
    ...
    const v = d.pct100 ? Math.min(100, raw) : (r ? normalize(raw, r.min, r.max) : null);
    if (v == null) continue;
    sum += v; n++;
  }
  ...
  return (
    <div className={`clubviral${compact ? " clubviral--compact" : ""}`}>
      ...
      <div className="cv-legend">
        <span className="cv-leg cv-leg--club">Club {cv.clubPct}%<span className="cv-leg-sub">RA · Beatport · 1001TL</span></span>
